...
Constraint.prototype.enforce = function() {
  ...
  let subVect = new THREE.Vector3().subVectors(this.p2.position, this.p1.position);
  let subLen = subVect.length();
  let correction = subVect.normalize().multiplyScalar((subLen - this.distance)/2);
...
function Cloth(w, h, l) {
  ...
  if (SceneParams.bendingSprings) {
    ...
    for (let v = 0; v <= h; v++) {
      for (let u = 0; u <= w; u++) {
        if (v < h-1){
          constraints.push(new Constraint(particles[index(u,v)], particles[index(u, v+2)], this.restDistanceB * this.restDistance));
        }
        if (u < w-1){
          constraints.push(new Constraint(particles[index(u,v)], particles[index(u+2, v)], this.restDistanceB * this.restDistance));
        }
      }
    }
    // ----------- STUDENT CODE END ------------
  ...
  if (!Cloth.eventHandlerRegistered) {
    ...
    window.addEventListener("keydown", handleImpactEvents, true);
    ...
    let mouseDownEvent = function(event){
      this.particle = cloth.getLookedAtParticle();
      if (this.particle) {
        this.selectedDist = this.particle.position.distanceTo(Scene.camera.position);
      }
    };
    let mouseMoveEvent = function(event){
      if (event.shiftKey && this.particle) {
        this.particle.position = Renderer.raycaster.ray.direction.clone().multiplyScalar(this.selectedDist).add(Scene.camera.position);
      }
    };
...
function handleImpactEvents(event) {
  ...
  let vec = keyMap[event.key];
  console.log(vec);
  
  if (particle != null && vec != null){
    
    particle.position.add(keyMap[event.key].multiplyScalar(scale));
  }
...
Cloth.prototype.applyGravity = function() {
  ...
  for (let p of particles){
    p.addForce(new THREE.Vector3(0, -p.mass * GRAVITY, 0));
  }
  // ----------- STUDENT CODE END ------------
};
...
Cloth.prototype.applyWind = function(windStrength) {
  ...
  let scale = 0.001;
  let st = 2+Math.cos(time * scale);
  let windForce = new THREE.Vector3(5*Math.cos(time*scale), 3*Math.sin(time*scale), 2*Math.cos(time*scale)).normalize().multiplyScalar(windStrength);
  windForce.multiplyScalar(st);
...
Cloth.prototype.applyRain = function(strength, rate) {
  ...
  // ----------- STUDENT CODE BEGIN ------------
  let impulse =  new THREE.Vector3(0, -strength, 0);
  let impusleWeak = impulse.clone().multiplyScalar(0.15);
  
  for (let i = 0; i < rate; i++){
    let hit = Sim.randomCoord();
    
    this.particles[this.index(hit.x, hit.y)].position.add(impulse);

    this.particles[this.index(hit.x-1, hit.y)].position.add(impusleWeak);
    this.particles[this.index(hit.x+1, hit.y)].position.add(impusleWeak);
    this.particles[this.index(hit.x, hit.y-1)].position.add(impusleWeak);
    this.particles[this.index(hit.x, hit.y+1)].position.add(impusleWeak);
    this.particles[this.index(hit.x-1, hit.y-1)].position.add(impusleWeak);
    this.particles[this.index(hit.x-1, hit.y+1)].position.add(impusleWeak);
    this.particles[this.index(hit.x+1, hit.y-1)].position.add(impusleWeak);
    this.particles[this.index(hit.x+1, hit.y+1)].position.add(impusleWeak);
  }
